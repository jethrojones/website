...
CLIENT_FILE = ROOT / ".gsc-oauth-client.json"
...
def load_credentials() -> Credentials:
    if not CLIENT_FILE.exists():
        raise SystemExit(f"Missing {CLIENT_FILE.name}")
    if not TOKEN_FILE.exists():
        raise SystemExit(f"Missing {TOKEN_FILE.name}")
...
def public_status(url: str) -> tuple[int | None, str | None]:
    command = ["curl", "-sS", "-I", "--max-time", "20", url]
    try:
...
def print_sitemap_status(service) -> None:
    ...
    for site in (SITE, OLD_SITE):
        ...
        except Exception as error:  # Google client wraps detailed API errors.
            print(f"  {site}: ERROR {type(error).__name__}: {error}")
            continue
...
def print_url_inspection(service) -> None:
    ...
    for url in ("https://jethrojones.com/", "https://jethrojones.com/inspirational-videos"):
        try:
            ...
            print(f"  {url}: ERROR {type(error).__name__}: {error}")
            continue
